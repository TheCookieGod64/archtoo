...
/* Captures stdout (stderr is discarded). Caller frees *out even on
   failure; *out is NULL only if the helper itself could not start.
   Returns the command's exit code, or -1 on fork/pipe errors. */
int run_cmd_capture(const char *cmd, char **out);

/* Single-quotes `in` for /bin/sh. Returns 0 if it does not fit. */
int shell_quote(const char *in, char *out, size_t n);

/* Search strings: letters, digits, space and @._+-/ only. */
int valid_search_query(const char *s);

/* Copies the package/provide name from a dependency token, stripping
   version constraints (>=, <=, =, >, <) and epoch (:). */
...
void set_emerge_confirm(int v);
int  get_emerge_confirm(void);
