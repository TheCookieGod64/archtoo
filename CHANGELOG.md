...
- Replace the v2 command stubs with real implementations: search, info,
  query, PKGBUILD fetch, local build, orphans, cache clean, stats, news,
  completion, providers, dependency planning, review, and devel listing.
- `emerge -A` no longer prints pacman's "package not found" error before
  falling back to AUR RPC info.
- `emerge --providers` keeps only exact package names and real `Provides`
  tokens. Substring hits such as `hello-git` for query `hello` are dropped.
- `emerge --deps` prints Depends / MakeDepends / CheckDepends and a
  dependency-first graph order instead of a single package name.
- `emerge --clean` deletes leftover `/var/cache/pacman/pkg/download-*`
  fragments before `pacman -Sc --noconfirm`, so incomplete downloads no
  longer produce "Error reading fd" failures.
- `emerge --news` parses RSS `<item>` titles and skips the channel title.
- `emerge --stats` reports installed, explicit, foreign, orphan, and
  `@world` counts from captured pacman output rather than a shell pipeline.
- Add `--review`, and document `--providers`, `--deps`, `--devel`, and
  `--complete` in `--help`.
- Cycle errors from the dependency graph now name both packages involved.
